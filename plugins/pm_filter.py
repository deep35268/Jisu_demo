...
lock = asyncio.Lock()
import traceback
from fuzzywuzzy import fuzz, process  # ✅ fuzz ਵੀ import ਕੀਤਾ
...
# ============================================================
# 🆕 **IMPROVED SPELL CHECK (Database-based, 60% threshold)**
# ============================================================
...
async def get_all_file_names():
    """Returns a list of distinct file names from DB with caching."""
    global _file_names_cache, _cache_time
    # Refresh cache every 5 minutes (300 seconds)
    if not _file_names_cache or (time.time() - _cache_time) > 300:
        try:
            # Fetch distinct file names from the Media collection
            all_names = await Media.collection.distinct("file_name")
            if all_names:
                _file_names_cache = all_names
            else:
                _file_names_cache = []
            _cache_time = time.time()
        except Exception as e:
            print(f"Error fetching distinct file names: {e}")
            _file_names_cache = []
    return _file_names_cache

async def ai_spell_check(wrong_name):
    """
    Uses fuzzy matching on actual file names from your DB (MongoDB).
    Returns the corrected movie name if a close match (>=60%) is found.
    Handles short words like 'kfg' -> 'kgf' perfectly.
    """
    all_names = await get_all_file_names()
    if not all_names:
        return None

    # Use fuzzywuzzy to find best matches (limit=5, threshold=60%)
    matches = process.extract(wrong_name, all_names, scorer=fuzz.ratio, limit=5)
    
    # Filter matches with score >= 60
    best_match = None
    for match, score in matches:
        if score >= 60:  # ✅ 60% threshold works great for 3-letter words
            best_match = match
            break  # take the highest scoring match
    
    if best_match:
        # Clean the matched name (remove extensions, year, brackets)
        cleaned = re.sub(r"\s*[\(\[]\d{4}[\)\]]?\s*", "", best_match)  # remove year
        cleaned = re.sub(r"\s*[\[\(].*?[\]\)]\s*", "", cleaned)        # remove brackets
        cleaned = cleaned.replace("_", " ").replace(".", " ").strip()
        # Further clean: keep only first part if too long, or return as is
        # For short queries, return the first few words
        words = cleaned.split()
        if len(words) > 3:
            cleaned = " ".join(words[:3])  # take first 3 words
        return cleaned
    return None
...
# ============================================================
# 🆕 **IMPROVED AUTO FILTER (Spell Check integrated)**
# ============================================================
