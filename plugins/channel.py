...
from plugins.helper.Imdbposter import get_movie_detailsx, get_movie_details
...
async def _process_with_lock(bot, filename, caption, media_info, base_name, processed):
    ...
    if not movie_doc:
        # Get metadata from TMDB/IMDB (for poster generation)
        if TMDB_POSTER:
        ...
        movie_doc = {
            "_id": base_name,
            "files": [file_data],
            "poster_url": details.get("poster_url"),  # not used now, kept for fallback
            "genres": genres,
            ...
            "message_id": None,
            "is_photo": False,
            # Save extra details for poster generation
            "plot": details.get("plot"),
            "runtime": details.get("runtime"),
            "title": details.get("title", base_name),
            "kind": details.get("kind", "Movie")
        }
        try:
...
# 🆕 Modified send_movie_update to generate custom poster
async def send_movie_update(bot, base_name):
    ...
    for attempt in range(max_retries):
        try:
            ...
            try:
                # Prepare metadata for poster
                title = movie_doc.get("title", base_name)
                subtitle = movie_doc.get("kind", "MOVIE")
                rating = movie_doc.get("rating", "N/A")
                year_val = movie_doc.get("year")
                duration = movie_doc.get("runtime", "2H25M")
                genres_str = movie_doc.get("genres", "Action / Drama")
                if isinstance(genres_str, str):
                    genres = [g.strip() for g in genres_str.split(",") if g.strip()]
                else:
                    genres = ["Action", "Drama"]
                plot = movie_doc.get("plot", None)
                ...
                logger.info(f"✅ Custom poster generated for {base_name}")
                ...
                logger.error(f"Poster generation failed: {e}, falling back to text")
